# `ai-config-kit` CLI reference

```
ai-config-kit [GLOBAL FLAGS] COMMAND [COMMAND FLAGS]
```
...
```bash
ai-config-kit bootstrap [--push] [--remote URL] [--no-git] [--with-settings] [-m MSG] [--dry-run]
```
...
```bash
ai-config-kit init [--no-examples] [--no-git] [--with-settings] [--save]
```
...
```bash
ai-config-kit compose-agents-md [-o PATH] [--dry-run]
```
...
output starts with `<!-- ai-config-kit:autogenerated -->`. Future
...
```bash
ai-config-kit project-install <project-path> \
...
```bash
ai-config-kit uninstall [--no-restore]
```
...
```bash
ai-config-kit sync [-m MESSAGE] [--push]
```
...
```bash
ai-config-kit heal [--yes] [--dry-run]
```
...
```bash
ai-config-kit capacity [--timezone IANA_ZONE] [--provider NAME ...]
```
...
```bash
ai-config-kit view <relpath> [--with-line-numbers]
```
...
```bash
ai-config-kit fetch <url> <dest> [--expect-sha256 HEX] [--max-bytes N]
...
```bash
ai-config-kit cleanup [--apply] [--include-ephemeral]
```
...
```bash
ai-config-kit decisions list
ai-config-kit decisions show <name>
ai-config-kit decisions apply <name> [--force] [--dry-run]
```
