...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        epilog=(
            "Common flow: `ai-config-kit bootstrap` covers init + install + doctor in "
            "one shot.\n\n"
    ...
    p.add_argument(
        "-V", "--version",
        action="version",
        version=f"ai-config-kit {__version__}",
    )
    ...
    p_recon = sub.add_parser(
        ...
        description=(
            "Compares the installed ai-config-kit version against the "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # When a newer ai-config-kit added packs to
        ...
        if args.cmd == "bootstrap":
            ...
            if boot_report.ok:
                ...
                _print("  - `ai-config-kit status` shows what's tracked", args.quiet)
                _print("  - `ai-config-kit sync` commits changes", args.quiet)
        ...
        if args.cmd == "init":
            ...
            _print("  next: ai-config-kit install", args.quiet)
            return 0
