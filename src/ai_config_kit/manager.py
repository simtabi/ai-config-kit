...
# Decisions data types live in ai_config_kit.decisions (C1 refactor).
# Re-imported here to preserve the historical module-level API.
...
class ListingReport:
    ...
    def to_text(self, max_per_group: int = 20) -> str:
        ...
        for group in self.groups:
            ...
            lines.append("")
        if not any_content:
            lines.append("(content dir is empty: try `ai-config-kit init`)")
        return "\n".join(lines).rstrip()
...
@dataclass(frozen=True)
class ComposeReport:
    """Outcome of ``compose_agents_md`` for a single content dir.

    ``inputs`` lists the source files concatenated, in order. ``output``
    is the path written (or that would be written under dry_run).
    """
    ...
    def summary(self) -> str:
        parts = [
            f"composed {self.output.name}",
            f"from {len(self.inputs)} input(s)",
...
def _package_version() -> str:
    """Currently-installed ``ai-config-kit`` version, or ``0.0.0`` if unknown."""
    try:
        from importlib.metadata import version as _v

        return _v("ai-config-kit")
    except Exception:
        return "0.0.0"
...
def _user_agent() -> str:
    ...
    return f"ai-config-kit/{_package_version()}"
...
class ClaudeConfig:
    ...
    ) / "ai-config-kit" / "config.json"
    DEFAULT_CONTENT: Path = Path(
        os.environ.get("XDG_CONFIG_HOME") or str(Path.home() / ".config")
    ) / "ai-config-kit" / "content"
    ...
    # Marker on the composed file so future re-compositions know they
    # can overwrite without losing user content. Users who hand-edit
    # AGENTS.md should remove this marker (then the configurator refuses
    # to overwrite).
    AGENTS_MD_AUTOGEN_MARKER: ClassVar[str] = (
    ...
    def compose_agents_md(
        ...
        """Synthesize a single ``AGENTS.md`` from the content dir.

        Order of concatenation:

        1. The user's ``src_dir/CLAUDE.md`` (if present).
        2. Every ``src_dir/CLAUDE.md.*.fragment`` file, sorted by name
           for stable output.

        Writes to ``out_path`` (default: ``src_dir/AGENTS.md``). The
        output starts with an autogen marker so the next compose call
        can detect it. If the existing file is missing the marker (i.e.
        the user hand-edited it), this method refuses to overwrite and
        raises ``ConfigError``.
        """
        target = (
            Path(out_path).expanduser().resolve()
            if out_path is not None
            else self.src_dir / "AGENTS.md"
        )

        # Refuse to clobber a hand-edited file.
        if target.exists():
            existing = target.read_text(encoding="utf-8", errors="replace")
            if self.AGENTS_MD_AUTOGEN_MARKER not in existing:
                raise ConfigError(
                    f"{target} exists but lacks the autogen marker; "
                    f"refusing to overwrite a hand-edited file. "
                    f"Remove or rename it first."
                )

        inputs: list[str] = []
        parts: list[str] = [
            self.AGENTS_MD_AUTOGEN_MARKER,
            "",
            "# AGENTS.md",
            "",
            (
                "Cross-vendor rules for any AI coding agent reading this "
                "file (aider, cursor, windsurf, copilot, codex, cline). "
                "Composed by `ai-config-kit compose-agents-md` from "
                "your CLAUDE.md plus the bundled decision-pack fragments."
            ),
            "",
        ]

        main = self.src_dir / "CLAUDE.md"
        if main.is_file():
        ...
        # Decision-pack fragments live as src_dir/CLAUDE.md.<pack>.fragment
        # after init applied them. Concatenate in name order for stability.
        fragments = sorted(self.src_dir.glob("CLAUDE.md.*.fragment"))
        for f in fragments:
    ...
    def project_install(
        ...
        (typically ``src_dir/AGENTS.md``). When ``auto_compose=True``
        (default) and a vendor's canonical source is missing, the
        configurator composes it on the fly from the user's CLAUDE.md +
        decision-pack fragments (idempotent).

        Refuses to overwrite an existing per-project file unless
        ``force`` is set, and surfaces unknown vendors as failures
        ...
        # Auto-compose AGENTS.md if any selected vendor needs it and it
        # isn't on disk yet. One pass before the per-vendor loop so the
        ...
        if auto_compose and not dry_run:
            ...
            if need_compose:
                with contextlib.suppress(ConfigError):
                    # Compose may refuse if the user has a hand-edited
                    # AGENTS.md without the autogen marker; in that case
                    # leave it alone and let the per-vendor copy step
                    # surface a failure if the file truly isn't there.
                    self.compose_agents_md()
    ...
    def _load_off_peak_data(
        ...
        2. The packaged copy under
           ``ai_config_kit/resources/decisions/model-overload-resilience/files/data/``.
    ...
    def _decisions_root(self) -> Any:
        ...
        return resources.files("ai_config_kit") / "resources" / "decisions"
    ...
    def reconcile(self, force_reapply: bool = False) -> ReconcileReport:
        ...
        3. If they differ (or ``force_reapply``), apply every pack in
           ...
           files stay; new files (added in a newer ``ai-config-kit``)
           land.
