...
```bash
ai-config-kit decisions apply humanistic-style
```
...
ai-config-kit sync -m "adopt humanistic-style"
```

Auto-applied by `ai-config-kit init`. The `banned-phrases.txt` ships
