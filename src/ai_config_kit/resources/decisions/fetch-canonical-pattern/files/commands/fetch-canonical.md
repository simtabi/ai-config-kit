...
**Preferred**: `ai-config-kit fetch` (Python, ships with ai-config-kit):

```bash
ai-config-kit fetch $1 $2
```

**Fallback** when ai-config-kit isn't installed but `curl` is:
...
**If you can't run `ai-config-kit fetch` and `curl` isn't available
