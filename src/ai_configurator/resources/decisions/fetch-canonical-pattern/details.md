...
```bash
ai-configurator decisions apply fetch-canonical-pattern
```
...
ai-configurator sync -m "adopt fetch-canonical-pattern"
```
...
| Tool | When |
|---|---|
| `ai-configurator fetch <url> <dest>` | Default. Pure Python, works on Windows, macOS, Linux. No external dependencies beyond `ai-configurator` itself. |
| `curl -fsSL -o <dest> <url>` | When `curl` is already on PATH and `ai-configurator` isn't (CI image without it, plain shell session, etc.). Same disk-to-disk behaviour. |
