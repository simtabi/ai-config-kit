...
versions of `ai-configurator` may extend it; the **reconcile**
mechanism applies those extensions to existing users on the first
command after `pip install --upgrade ai-configurator`.
...
```bash
ai-configurator decisions apply claude-best-practices
```
...
ai-configurator sync -m "adopt claude-best-practices"
```

## Why this auto-reconciles

`ai-configurator` ships new versions as Claude Code's docs evolve.
...
`ai-configurator` command you'll see:
